...
# if 'IVITestDataset' in DATASET_REGISTRY._obj_map:
#     del DATASET_REGISTRY._obj_map['IVITestDataset']
...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if 'meta_info_file' in opt:
            ...
            subfolders_ir_lq = natsorted(glob.glob(osp.join(self.ir_lq_root, '*'))) ## 遍历self.lq_root下所有子文件夹 并排序 结果是一个路径列表 而不是文件夹名 列表
            subfolders_ir_gt = natsorted(glob.glob(osp.join(self.ir_gt_root, '*'))) ## 遍历self.gt_root下所有子文件夹 并排序
            subfolders_vi_lq = natsorted(glob.glob(osp.join(self.vi_lq_root, '*'))) ## 遍历self.lq_root下所有子文件夹 并排序 结果是一个路径列表 而不是文件夹名 列表
            subfolders_vi_gt = natsorted(glob.glob(osp.join(self.vi_gt_root, '*'))) ## 遍历self.gt_root下所有子文件夹 并排序

        if opt['name'].lower() in ['reds', 'gopro', 'dvd', 'bsd', 'real', 'compress', 'hdo', 'm3sv', 'm3svd', 'tno']:
            for subfolder_ir_lq, subfolder_ir_gt, subfolder_vi_lq, subfolder_vi_gt in zip(subfolders_ir_lq, subfolders_ir_gt, subfolders_vi_lq, subfolders_vi_gt):
                ...
                ir_img_paths_lq = natsorted(list(scandir(subfolder_ir_lq, full_path=True))) ## 遍历subfolder_lq下所有文件或文件夹路径，返回一个路径列表
                ir_img_paths_gt = natsorted(list(scandir(subfolder_ir_gt, full_path=True)))

                vi_img_paths_lq = natsorted(list(scandir(subfolder_vi_lq, full_path=True))) ## 遍历subfolder_lq下所有文件或文件夹路径，返回一个路径列表
                vi_img_paths_gt = natsorted(list(scandir(subfolder_vi_gt, full_path=True)))
...
class VideoFusionTestDataset(VideoTestDataset):
    ...
    def __init__(self, opt):
        ...
        for folder in ori_folders:
            ...
            ir_ori_imgs_lq_paths[folder] = natsorted(self.data_info['ir_lq_path'][now_idx:now_idx + nf])
            ir_ori_imgs_gt_paths[folder] = natsorted(self.data_info['ir_gt_path'][now_idx:now_idx + nf])
            vi_ori_imgs_lq_paths[folder] = natsorted(self.data_info['vi_lq_path'][now_idx:now_idx + nf])
            vi_ori_imgs_gt_paths[folder] = natsorted(self.data_info['vi_gt_path'][now_idx:now_idx + nf])
            now_idx = now_idx + nf
        ...
        for folder in ori_folders:
            ...
            if (num_all - num_overlap) % (num_frame - num_overlap) != 0:
                ...
                clip_folders.append(clip_folder)
                ir_clip_imgs_lq[clip_folder] = self.imgs_ir_lq[folder][-num_frame:]
                ir_clip_imgs_gt[clip_folder] = self.imgs_ir_gt[folder][-num_frame:]
                vi_clip_imgs_lq[clip_folder] = self.imgs_vi_lq[folder][-num_frame:]
                vi_clip_imgs_gt[clip_folder] = self.imgs_vi_gt[folder][-num_frame:]
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            names = [os.path.basename(p) for p in ir_img_paths_lq]
